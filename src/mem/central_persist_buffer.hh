...
class CentralPersistBuffer : public ClockedObject
{
  ...
  public:
    ...
    CentralPersistBuffer(Params* params) :
        ...
        pbCapacity(params->pbCapacity), flushThreshold(params->flushThreshold),
        flushInterval(params->flushInterval), flushingStarted(false),
        pollLatency(params->pollLatency), regFlushEvent(*this)
    {
  ...
  private:
    ...
    void recvReqRetry(PortID idx)
    {
        ...
        if (idx >= numThreads && memSaturated[idx-numThreads]) {
            DPRINTF(PersistBuffer, "Recieved retry from portID:%d\n", idx);
