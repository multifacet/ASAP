...
sudo build/X86/gem5.fast -d results/ideal/$1 configs/example/fs.py --pmem --pmem-pwq --persist-buffers --pmem-wr-latency=60ns --mem-size=48GB --cpu-type=X86KvmCPU --kernel=$(pwd)/vmlinux_12 --disk-image=$image --ruby --asap -n $cores --script=scripts/$1.rcS
